...
String sonde1 = "";                                      //String where the data is saved (Conductivity)   
String sonde2 = "";                                      //String where the data is saved (D.O.)    
String sonde3 = "";                                      //String where the data is saved (PH)   
...
void loop()
{
  ...
  if (sondeComplete1)                                    //Verifying if the data from Conductivity is stored.  
  {
    if (sondeComplete2)                                  //Verifying if the data from D.O. is stored.
    {
      ...
      { 
        ...
        Serial1.print("Id:");
        Serial1.println(serie);                          //Writing the number of SMS
        Serial1.print("Name:");
        ...
        Serial1.print("TempC:");
        Serial1.println(sensors.getTempCByIndex(0));     //Command to insert the temperature data in the SMS 
        sonde1.trim(); 
        if (sonde1.substring(0,2) == comp)               //Verification of the conductivity data
        { 
        ...
        { 
          ...
          Serial1.println(sonde1);                         //Command to insert the Conductivity data in the SMS  
          Serial1.println("testtest");
          Serial1.println("testtest");
        }
        delay(300);
        sonde1 = "";
      ...
      {
        myserial.print(sensors.getTempCByIndex(0));      //Sending the temperature to the PH sensor
        myserial.print("\r");
    ...
    {
      Serial2.print(sensors.getTempCByIndex(0));         //Sending the temperature to the D.O. sensor
      Serial2.print("\r");
      delay(1500);                                       //Give the sensor some thinking time
    } 
  }
  ...
  {
    Serial3.print(sensors.getTempCByIndex(0));           //Sending the temperature to the PH sensor
    Serial3.print("\r");
    delay(1500);                                         //Give the sensor some thinking time
  }
}        
